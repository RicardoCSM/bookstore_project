  ...
  <div>
    <form @submit.prevent="submitForm"
      ...
      <div class="mb-2">
        ...
        <input type="text" name="address-identification" id="address-identification" v-model="addressIdentification"
          class="w-full px-3 py-2 border border-gray-400 rounded" placeholder="Enter your address identification"
          required>
      </div>
      <div class="mb-2">
        <label for="street-name" class="block mb-2 font-bold text-gray-700">Street Name</label>
        <input type="text" name="street-name" id="street-name" v-model="streetName"
          class="w-full px-3 py-2 border border-gray-400 rounded" placeholder="Enter your street name" required>
      </div>
      <div class="mb-2">
        <label for="street-number" class="block mb-2 font-bold text-gray-700">Street Number</label>
        <input type="text" name="street-number" id="street-number" v-model="streetNumber"
          class="w-full px-3 py-2 border border-gray-400 rounded" placeholder="Enter your street number" required>
      </div>
      <div class="mb-2">
        <label for="zip-code" class="block mb-2 font-bold text-gray-700">Zip Code</label>
        <input type="text" name="zip-code" id="zip-code" v-model="zipCode"
          class="w-full px-3 py-2 border border-gray-400 rounded" placeholder="Enter your zip code" required>
      </div>
      <div class="mb-2">
        ...
        <input type="text" name="complement" id="complement" v-model="complement"
          class="w-full px-3 py-2 border border-gray-400 rounded" placeholder="Enter any additional information">
      </div>
    ...
    const submitForm = () => {
      ...
      axios.post('/api/addresses', {
        ...
        .catch(error => {
          ...
          modalMessage.value = error.response?.data?.message || 'An error occurred while creating your address.'
          showModal.value = true
        })
    }
