<template>
    <div class="block sm:hidden">
        <button @click="toggleNavbar()"
            class="flex items-center px-3 py-2 text-white border border-white rounded text-teal-lighter">
            <i class="fas fa-bars"></i>
        </button>
    </div>
    <div :class="{ 'hidden': !this.showMenu, 'flex': this.showMenu }"
        class="flex-col items-start w-full sm:flex-row sm:flex sm:items-center sm:w-auto">
        ...
        <div class="mt-4 sm:items-center sm:ml-6 sm:mt-0 sm:flex">
            ...
            <div v-if="auth.user" class="flex justify-center mt-3 sm:mt-0">
                <NavBarDropdown :button-label="auth.user.name" />
            </div>
            ...
            <div v-else class="flex justify-center mt-3 ml-1 sm:mt-0">
                ...
                <a href="/register"
                    class="px-3 py-2 ml-4 text-sm font-medium text-white bg-blue-500 rounded-md hover:bg-blue-600">Register</a>
            </div>
        </div>
    </div>
</template>
