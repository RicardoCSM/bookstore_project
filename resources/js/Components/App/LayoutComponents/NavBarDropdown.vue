<template>
  <div class="relative z-20 inline-block text-right sm:ml-1">
    <button id="dropdownDefaultButton" data-dropdown-toggle="dropdown" @click="isOpen = !isOpen"
      class="text-white bg-blue-500 hover:bg-blue-700 focus:ring-4 focus:outline-none focus:ring-blue-800 font-medium rounded-lg text-sm px-4 py-2.5 text-center inline-flex items-center"
      type="button">
      {{ buttonLabel }}
    ...
    <div v-show="isOpen" id="dropdown"
      class="absolute right-0 z-10 mt-2 bg-gray-700 divide-y divide-gray-100 rounded-lg shadow w-44" role="menu"
      ...
      <ul class="py-2 text-sm text-gray-200">
        ...
        <li>
          <button type="button" @click.prevent="logOut()"
            class="block w-full px-4 py-2 text-right hover:bg-gray-600 hover:text-white" role="menuitem">Sign out</button>
        </li>
      </ul>
    </div>
  </div>
