...
import AddressForm from '@/Components/App/OrderComponents/AddressForm.vue';
import ConfirmOrderButton from '@/Components/App/OrderComponents/ConfirmOrderButton.vue';
...
const selectedAddressInfo = computed(() => {
  ...
  const address = (addresses.value as any).find((a) => a.id === selectedAddress.value.id);
...
<template>
  ...
  <div class="relative min-h-screen bg-gray-900 selection:bg-blue-500 selection:text-white">
    <NavBar />
        ...
        <div class="flex items-center justify-between px-4 py-6 m-5 bg-white border border-gray-300 rounded-lg shadow-lg">
          <div class="w-1/2 pr-6">
            <select v-model="selectedAddress"
              class="block w-full px-4 py-2 leading-tight text-gray-700 bg-white border border-gray-400 rounded-lg appearance-none focus:outline-none focus:border-gray-500">
              <option value="">Select an address</option>
              <option v-for="address in addresses" :value="address">{{ address.identification }}</option>
            </select>
            <button type="button" @click.prevent="changeContent()"
              class="inline-block px-4 py-2 mt-4 font-bold text-white bg-indigo-500 rounded-lg hover:bg-indigo-700 focus:outline-none focus:shadow-outline-indigo">
              Add address
            </button>
          </div>
          ...
          <div class="w-1/3 px-6">
            ...
            <p class="mb-2 text-gray-700">{{ book.isbn_13 }}</p>
          </div>
        </div>
